...
if ($user_exists) {
    // Update akun yang sudah ada tapi belum terverifikasi
    $stmt = $conn->prepare('UPDATE user SET nama = ?, pass = ?, verification_code = ?, code_expires_at = ? WHERE id_user = ?');
    $stmt->bind_param('ssssi', $nama, $hash, $otp, $expires, $user_exists['id_user']);
} else {
    // Insert baru
